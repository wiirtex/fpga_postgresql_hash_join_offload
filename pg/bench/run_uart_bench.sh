...
DEVICE="${FPGA_DEVICE:-/dev/ttyUSB1}"
...
OUT="$RESULT_DIR/uart_bench_${STAMP}.txt"
...
{
  echo "UART benchmark started at $(date --iso-8601=seconds)"
  echo "device=$DEVICE baud=$BAUD db=$DB"
  echo "mode=uart"
  echo "transport=uart"
  echo "algorithm=a"
...
for case in "${cases[@]}"; do
  ...
  for ((run_id = 1; run_id <= total_runs; run_id++)); do
    ...
    "$PSQL" -d "$DB" -v ON_ERROR_STOP=1 \
      -v fpga_device="'$DEVICE'" \
      -v fpga_baud="$BAUD" \
      -v label="$label" \
      -v inner_n="$inner_n" \
      -v outer_n="$outer_n" \
      -v bench_pattern="'$pattern'" \
      -f "$SCRIPT_DIR/uart_bench.sql" 2>&1 | tee -a "$OUT"
  done
done
