...
HOST="${FPGA_UDP_HOST:-172.26.96.1:50001}"
ALGORITHM="${FPGA_ALGORITHM:-b}"
ACK_WINDOW="${FPGA_ACK_WINDOW:-15}"
MAX_BATCH="${FPGA_MAX_BATCH:-118}"
...
OUT="$RESULT_DIR/udp_bench_${STAMP}.txt"
...
{
  echo "UDP benchmark started at $(date --iso-8601=seconds)"
  echo "host=$HOST algorithm=$ALGORITHM db=$DB"
  echo "mode=udp"
  echo "transport=udp"
  echo "algorithm=$ALGORITHM"
  echo "ack_window=$ACK_WINDOW"
...
for case in "${cases[@]}"; do
  ...
  for ((run_id = 1; run_id <= total_runs; run_id++)); do
    ...
    FPGA_LOG_LEVEL="${FPGA_LOG_LEVEL:-WARN}" \
    "$PSQL" -d "$DB" -v ON_ERROR_STOP=1 \
      -v bench_mode="'udp'" \
      -v bench_algorithm="'$ALGORITHM'" \
      -v fpga_ack_window="$ACK_WINDOW" \
      -v fpga_max_batch="$MAX_BATCH" \
      -v fpga_device="'$HOST'" \
      -v fpga_baud="115200" \
      -v label="$label" \
      -v inner_n="$inner_n" \
